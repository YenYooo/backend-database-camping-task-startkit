...
WHERE "USER".email = 'wXlTq@hexschooltest.io' AND cancelled_at  IS NOT NULL
...
SELECT 
	"CREDIT_PURCHASE".user_id AS user_id ,
	 ("CREDIT_PURCHASE".total_credit - "COURSE_BOOKING".used_credit) as remaining_credit -- 用戶王小明的剩餘可用堂數
FROM
	(SELECT 
		user_id,
...
WHERE EXTRACT(MONTH FROM "CREDIT_PURCHASE".purchase_at) = 11 -- 11月銷售
...
FROM "CREDIT_PURCHASE" 
GROUP BY EXTRACT(MONTH FROM purchase_at) = 11; -- 11月銷售

-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
SELECT 
	DISTINCT COUNT(user_id) AS 預約會員人數
FROM "COURSE_BOOKING" 
